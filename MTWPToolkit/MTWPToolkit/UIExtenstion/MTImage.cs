...
namespace Meituan.Client.Controls.CustomControl {
    public class MTImage {
        ...
        private static readonly object IsoLock = new object();
        #endregion
            ...
            try{
                ...
                using(var stream = await response.Content.ReadAsStreamAsync()) {
                    lock(IsoLock) {
                        ...
                        //将下载的图片缓存到独立存储区中
                        if(!IsolatedStorage.FileExists(GetCacheFilePath(imageurl))) {
        ...
        private static bool CacheImageExists(string imageurl) {
            ...
            lock(IsolatedStorage) {
                isfileExists = IsolatedStorage.FileExists(GetCacheFilePath(imageurl));
            }
            return isfileExists;
        }
